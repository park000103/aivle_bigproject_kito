...
VOICEFLOW_API ='VF.DM.667a7b6b6713dc7c12c4b6b4.TcI959ncBBs9W6gW'  # Voiceflow API 키 넣기
    ...
    if request.method == "POST":
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                messages = []
                 # 데이터를 순회하면서 순서대로 messages에 추가하고 index도 함께 저장
                for index, item in enumerate(data):
                    if item['type'] == 'text' and 'message' in item['payload']:
                        message = item['payload']['message']
                        delay = item['payload'].get('delay', 0)
                        messages.append({'type': 'text', 'content': message, 'delay': delay, 'index': index})
                    elif item['type'] == 'visual' and 'image' in item['payload']:
                        image = item['payload']['image']
                        messages.append({'type': 'image', 'content': image, 'delay': 0, 'index': index})

                # messages를 index 기준으로 정렬
